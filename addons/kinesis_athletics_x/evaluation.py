...
class evaluation(models.Model):
    ...
    def _get_partner_age(self):
        self.age = self.partner_id.age

    eval_year = fields.Integer(
        compute='_calc_date',
        string="Year",
        store=True)
    ...
    age = fields.Integer(
        ...
        string="Age",
        store=True)
    evaluation_detail_value_ids = fields.One2many(
        'kinesis_athletics.evaluation_detail',
        'evaluation_id',
        string='Values',
        domain=[('test_type', '=', 'value')],
        readonly=True)
    evaluation_detail_selection_ids = fields.One2many(
        'kinesis_athletics.evaluation_detail',
        'evaluation_id',
        string='Selections',
        domain=[('test_type', '=', 'selection')],
        readonly=True)
    ...
    def copy(self, cr, uid, id, default, context=None):
        ...
        res = super(evaluation, self).copy(
